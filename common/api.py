...
def edit_listing(id, address, price, baths, beds, size, description, property_type, amenities, author, tag, galleryUrl, photo=None):
    # Define Listing
    l = Listing()
    ...
    l = db.get(key)
    logging.info("Current Object: %s" % l)
    
    # Check Author
    if not author:
        logging.warn("Invalid user: %s" % author)
        raise exception.ApiOwnerRequired('Invalid user: %s' % author)
    ...
    # Store Object
    l.put()
    logging.info("Listing: %s" % l)
    return l

  
def add_listing(address, price, baths, beds, size, description, property_type, amenities, author, tag, galleryUrl, photo=None):
    # Define Listing
    l = Listing()
